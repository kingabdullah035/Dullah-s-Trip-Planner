...
export default function TripPage(){
  ...
  React.useEffect(()=>{
    api(`/trips/${id}`).then(setTrip)
    api(`/trips/${id}/messages`).then(setMessages)
  }, [id])

  async function sendStream(){
    if(!input.trim() || isStreaming) return
    setErr(''); setStreamingText(''); setIsStreaming(true)

    try {
      const token = localStorage.getItem('supabase_token') || localStorage.getItem('token') || ''
      const res = await fetch('http://localhost:4000/chat/stream', {
        method: 'POST',
        headers: {
      ...
      if(!res.ok || !res.body){
        setErr('Stream failed.'); setIsStreaming(false); return
      }
        ...
        for (const line of lines){
          ...
          try{
            ...
            if (j.done) {
              ...
              setTrip(t); setMessages(m)
            }
          }catch{}
        }
      }
    } catch (e) {
      setErr(String(e)); setIsStreaming(false)
    }
  }
  ...
  React.useEffect(()=>{
    if(!trip) return
    if(!mapRef.current){
      const m = L.map('map').setView([41.387,2.170], 13)
    ...
    if(trip.places?.length){
      ...
      if(group.getLayers().length) mapRef.current.fitBounds(group.getBounds().pad(0.2))
    }
    ...
    <>
      ...
      <section className="section">
        ...
        <div className="space-between mt-3">
          <input className="input" value={input} onChange={e=>setInput(e.target.value)} placeholder="Ask for a plan, add preferences..." />
